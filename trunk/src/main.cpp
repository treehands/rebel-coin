...
const int64 nChainStartTime = 1385331565; 
...
const string strMessageMagic = "rebelcoin Signed Message:\n";
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime)
{
    ...
    // rebelcoin: reward for coin-year is cut in half every 64x multiply of PoS difficulty
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal rebelcoin miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in rebelcoin
    bool fStrictPayToScriptHash = true; // Always active in rebelcoin
    ...
    // ppcoin: fees are not collected by miners as in rebelcoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "rebelcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        txNew.strTxComment = "text:rebelcoin genesis block";
        ...
        assert(block.hashMerkleRoot == uint256("0x16d53decd732574d2e02f3ce40ae402c23835780ca89b8e8efa2a618ea67ec5e"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("rebelcoinMiner : proof-of-work not meeting target");

    //// debug print
    printf("rebelcoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("rebelcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("rebelcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadrebelcoinMiner(void* parg);

static bool fGeneraterebelcoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void rebelcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    printf("CPUMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("rebelcoin-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGeneraterebelcoins || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGeneraterebelcoins) && !fProofOfStake)
                return;
        }
        ...
        printf("Running rebelcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadrebelcoinMiner(void* parg)
{
    ...
    try
    {
        ...
        rebelcoinMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadrebelcoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadrebelcoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadrebelcoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void Generaterebelcoins(bool fGenerate, CWallet* pwallet)
{
    fGeneraterebelcoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGeneraterebelcoins = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d rebelcoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadrebelcoinMiner, pwallet))
                printf("Error: NewThread(ThreadrebelcoinMiner) failed\n");
            Sleep(10);
        }
    }
}
